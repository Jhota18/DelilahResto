      ...
      requestsModel
        ...
        .then((request) => {
          Promise.all(
            products.map((product) => {
              return request.addProducts(product.id, {
            ...
            .catch((error) => {
              ...
              if (error.name === "SequelizeForeignKeyConstraintError") {
                rejc({
                  status: 404,
                  message:
                    "No se encontró el ID de producto, por favor verifique",
                });
              } else {
                rejc({
                  status: 500,
                  message: "Error interno, no se pudo crear la orden",
                });
              }
            });
        })
        .catch((error) => {
          ...
          if (error.name === "SequelizeForeignKeyConstraintError") {
            rejc({
              status: 404,
              message: "No se encontró el ID de usuario, por favor verifique",
            });
          }
        });
    } else {
      ...
      requestsModel
        ...
        .then((response) => {
          ...
          if (response[0] === 1) {
            ...
            rejc({
              status: 404,
              message: "Datos no encontrados, no se pudo actualizar tu pedido.",
            });
          }
        })
        .catch((error) => {
      ...
      requestsModel
        ...
        .then((response) => {
          if (response === 1) {
            ...
            rejc({
              status: 404,
              message: `No se encontro el pedido, por favor verifique`,
            });
          }
        })
        .catch((error) => {
          rejc({
            status: 500,
            message: "Tenemos problemas, por favor intente  mas tarde.",
          });
        });
    } else {
      rejc({ status: 400, message: "Por favor envie el ID del pedido" });
    }
  });
};

module.exports = { createRequest, updateById, deleteById };
